...
class InfraAgentSession:
    ...
    async def send_credentials_status(self):
        """Checks AWS credentials and sends the status to the client."""
        aws_keys = ["AWS_ACCESS_KEY_ID", "AWS_SECRET_ACCESS_KEY"]
        has_credentials = all(k in os.environ for k in aws_keys)
        await self.send_to_client({
    ...
    async def _run_agent(self, initial_prompt: str):
        try:
            # Check AWS Credentials and notify the UI of current availability
            aws_keys = ["AWS_ACCESS_KEY_ID", "AWS_SECRET_ACCESS_KEY"]
            has_credentials = all(k in os.environ for k in aws_keys)
            await self.send_to_client({
                "type": "credentials_status",
